...
uses
  ...
  UUsuario, URegraCRUDUsuario, UUtilitarios, PwCtrls;
...
type
  ...
  protected
    ...
    procedure HabilitaCampos(const ceTipoOperacaoUsuario
      : TTipoOperacaoUsuario); override;
...
uses
  UOpcaoPesquisa, StrUtils, UDialogo;
...
begin
  inherited;
  if (Trim(edSenha.Text) <> EmptyStr) and
    (Trim(edConfirmacaoSenha.Text) <> EmptyStr) then
    try
...
procedure TFrmCadastroUsuario.HabilitaCampos(const ceTipoOperacaoUsuario
  : TTipoOperacaoUsuario);
begin
  inherited;
  gbInformacoes.Enabled := ceTipoOperacaoUsuario
    in [touInsercao, touAtualizacao];
...
begin
  ...
  AdicionaOpcaoPesquisa(TOpcaoPesquisa.create.DefineVisao(VW_USUARIO)
    .DefineNomeCampoRetorno(VW_USUARIO_ID).AdicionaFiltro(VW_USUARIO_NOME)
    .DefineNomePesquisa('Por Nome'));

  AdicionaOpcaoPesquisa(TOpcaoPesquisa.create.DefineVisao(VW_USUARIO)
    .DefineNomeCampoRetorno(VW_USUARIO_ID).AdicionaFiltro(VW_USUARIO_LOGIN)
